...
module.exports = function() {
  ...
  server.command('sethome', async (event) => {
    try {
      ...
      if (homes[location.dimension].hasOwnProperty(home)) {
        ...
        if (Object.keys(homes[location.dimension]).length >= config.home.amount) {
  ...
  server.command('setwarp', async (event) => {
    try {
      ...
      const location = await server.util.getLocation(event.player);
      let warps = await server.JSON.get('world', 'warp') || {};
  ...
  function handler(e, player) {
    if (e.playerError) server.util.tellRaw(e.message, player, { color: 'red' });
    else server.util.tellRaw('Something went wrong, please let the server admin know of the following error:', player, { color: 'red' })
